@responsenoshufflevalidation
@entiretest

Feature: Verify the responses should not get shuffled
...
Scenario: 02 Change the Time Zone to UTC
 ...
 Scenario: 03 To upload an Excel in the Direct Import Page
    ...
    And The user upload the excel file "testingdata10.xlsx" in the import file field
...
Scenario: 09 Validate the message send should appear in the same leads inbox
   Given The user logs in the Zeitblast application with "super admin" credentials
    Then The User should be able to navigate the Inbox page
    And The User should observe the message send for the lead should appear only in the same leads inbox
    And The user logs out from the Zeitblast application
