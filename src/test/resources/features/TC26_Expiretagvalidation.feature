...
Feature: TC_26 If lead does not respond within a minute then it should move to expire tag(For the last Drip Scheduled Msg)
...
Scenario: 12 Validate the first drip message assigned to the lead
 Given The user logs in the Zeitblast application with "super admin" credentials
  When The User should be able to navigate the Inbox page
  Then The user should observe the first drip assigned to the lead
  And The user logs out from the Zeitblast application
  
Scenario: 13 Change the date as today for drip 2 in MongoDB
Given the user initializes the MongoDB connection
When change the date in drip second automation for today  
 Then the user closes the MongoDB connection
  
Scenario: 14 After waiting for a minute the expired tag should be assigned for the lead
 Given The user logs in the Zeitblast application with "super admin" credentials
  When The User should be able to navigate the Inbox page
  And The user should wait for a minute and observe a expired tag
  And The user logs out from the Zeitblast application
